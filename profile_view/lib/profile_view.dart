...
class _ProfileViewState extends State<ProfileView> {
  final step1Fromkey = GlobalKey<FormState>();
  ...
  Widget build(BuildContext context) {
    void updateStep(int value) {
      if (currentStep == 0) {
        if (step1Fromkey.currentState!.validate()) {
          step1Fromkey.currentState!.save();
          setState(() {
            currentStep = value;
          });
        }
        ...
        if (value > currentStep) {
          if (step2Fromkey.currentState!.validate()) {
            step2Fromkey.currentState!.save();
            setState(() {
              currentStep = value;
            });
          }
            ...
            return Stepper(
              ...
              steps: [
                Step(
                    ...
                    content:
                        Step1Form(formKey: step1Fromkey, userInfo: userInfo),
                    isActive: currentStep == 0),
                Step(
                    title: const Text('Địa chỉ'),
                    content:
                        Step2Form(formKey: step2Fromkey, userInfo: userInfo),
            ...
            TextFormField(
              ...
              validator: (value) {
                ...
                if (value == null || value.isEmpty) {
                  ...
                  return 'Định dang số điện thoại không hợp lệ';
                }
                return null;
              },
...
class _Step2FormState extends State<Step2Form> {
  ...
  Future<void> loadLocationData() async {
    try {
      ...
      // Trích xuất danh sách các tinh từ dữ Liệu từ đien và chuyển đổi thành
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
          ...
          _buildInfoItem('Số đien thoại:', userInfo.phoneNumber),
          _buildInfoItem('Tinh / Thanh phố:', userInfo.address?.province?.name),
          _buildInfoItem('Huyen / Quận:', userInfo.address?.district?.name),
          _buildInfoItem(
              'Xã / Phường / Thị trấn: ', userInfo.address?.ward?.name),
          _buildInfoItem('Dia chi:', userInfo.address?.street),
        ],
      ),
    );
  }
